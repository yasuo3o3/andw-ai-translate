...
class ANDW_AI_Translate_Meta_Box {
	...
	public function ajax_translate_post() {
		...
		$back_translation = $this->block_parser->retranslate_translated_content( $result['translated_content'], 'ja', $provider );
	...
	public function ajax_ab_compare() {
		...
		foreach ( array_slice( $provider_keys, 0, 2 ) as $provider ) {
			...
			$back_translation = $this->block_parser->retranslate_translated_content( $translation['translated_content'], 'ja', $provider );
